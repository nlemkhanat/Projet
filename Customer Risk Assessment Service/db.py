...
def get_approved_loan(loan_id):
    try:
        ...
        logging.error(f"Erreur lors de la récupération du prêt approuvé : {e}")
        raise

def insert_transfer_transaction(customer_id, amount):
    try:
        conn = get_db_connection()
        cur = conn.cursor()
        cur.execute("""
            INSERT INTO TransactionHistory (customer_id, transaction_date, amount, transaction_type)
            VALUES (?, datetime('now'), ?, 'TRANSFER')
        """, (customer_id, amount))
        conn.commit()
        conn.close()
        logging.info(f"Transfert enregistré pour customer_id {customer_id} avec amount {amount}")
    except sqlite3.Error as e:
...
def get_customer_by_id(customer_id):
    try:
        conn = get_db_connection()
        cur = conn.cursor()
        cur.execute("SELECT id, name, email FROM Customer WHERE id = ?", (customer_id,))
        result = cur.fetchone()
        conn.close()
        return result
    except sqlite3.Error as e:
        logging.error(f"Erreur lors de la récupération du client : {e}")
        raise
